...
function App() {
  ...
  return <div className="min-h-screen overflow-hidden bg-paper text-ink selection:bg-lime selection:text-ink">
    ...
    <main id="top">
      ...
      <section id="faq" className="mx-auto max-w-4xl px-5 py-24 lg:py-32"><div className="reveal text-center"><p className="eyebrow">Perguntas frequentes</p><h2 className="section-title">Ficou alguma<br/><span className="text-black/35">dúvida?</span></h2></div><div className="mt-12 divide-y divide-black/8 rounded-3xl border border-black/8 bg-white px-6">{faqs.map(([q,a],i)=><div key={q} className="reveal"><button className="flex w-full items-center justify-between gap-5 py-6 text-left font-bold" onClick={() => setOpenFaq(openFaq===i?null:i)}><span>{q}</span><ChevronDown className={`shrink-0 transition ${openFaq===i?'rotate-180':''}`} size={19}/></button><div className={`grid transition-all ${openFaq===i?'grid-rows-[1fr] pb-6':'grid-rows-[0fr]'}`}><p className="overflow-hidden text-sm leading-7 text-black/50">{a}</p></div></div>)}</div></section>
